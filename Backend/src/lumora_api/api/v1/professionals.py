...
@router.get("", response_model=Page[ProfessionalRead], summary="Listar profesionales")
...
@router.post("", response_model=ProfessionalRead, status_code=201, responses=ERRORS)
async def create_professional(data: ProfessionalCreate, session: SessionDep):
    return await service(session).create(data)


@router.get("/{professional_id}", response_model=ProfessionalRead, responses={404: ERRORS[404]})
async def get_professional(professional_id: int, session: SessionDep):
    return await service(session).get(professional_id)


@router.patch("/{professional_id}", response_model=ProfessionalRead, responses=ERRORS)
...
@router.delete("/{professional_id}", status_code=status.HTTP_204_NO_CONTENT, responses={404: ERRORS[404]})
