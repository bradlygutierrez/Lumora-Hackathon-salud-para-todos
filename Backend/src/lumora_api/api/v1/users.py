...
@router.post('/admin', response_model=UserRead, status_code=status.HTTP_201_CREATED, responses=ERRORS, dependencies=[Depends(require_permission('rbac:manage'))], summary='Crear cuenta de administrador')
...
@router.post('/{user_id}/password-reset', response_model=MessageResponse, responses={404: ERRORS[404]}, dependencies=[Depends(require_permission('rbac:manage'))], summary='Reenviar correo de cambio de contraseña')
