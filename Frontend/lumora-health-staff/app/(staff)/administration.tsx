...
export default function AdministrationScreen() {
  ...
  const canManage = permissions.has('rbac:manage');
  const queryClient = useQueryClient();
  // enabled: canManage -- sin esto, la query (y su error de red/permiso)
  // se dispara para cualquier usuario que monte esta pantalla, incluso
  // sin rbac:manage, mostrando "No se pudo cargar la administración"
  // antes de llegar siquiera al chequeo de permiso de abajo.
  const users = useQuery({ queryKey: ['administration', 'users'], queryFn: listUsers, enabled: canManage });
  ...
  if (!canManage) return <ErrorState title='Acceso restringido' />;
  ...
  return <Screen><ScrollView contentContainerStyle={styles.content} style={styles.scroll}>
