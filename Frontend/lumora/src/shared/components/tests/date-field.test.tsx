...
describe('DateField', () => {
  ...
  describe('date-only values in a timezone behind UTC', () => {
    const originalTZ = process.env.TZ;

    beforeEach(() => {
      // Regresión: `new Date('2005-10-20')` (fecha SIN hora) se interpreta
      // como medianoche UTC -- en cualquier huso detrás de UTC (Nicaragua
      // es UTC-6) eso cae en el día anterior al leerlo con getters locales,
      // mostrando y reabriendo el picker un día atrás del que se guardó.
    ...
    it('displays the stored day, not the day before', async () => {
      const screen = await render(
        <DateField label="Fecha de nacimiento" onChange={jest.fn()} value="2005-10-20" />,
      );
      expect(screen.getByText('20/10/2005')).toBeTruthy();
    });

    it('reopens the Android picker pre-filled with the stored day, not the day before', async () => {
      Platform.OS = 'android';
      const screen = await render(
        <DateField label="Fecha de nacimiento" onChange={jest.fn()} value="2005-10-20" />,
      );
      ...
      const openArgs = mockOpen.mock.calls[0][0];
      expect(openArgs.value.getFullYear()).toBe(2005);
      expect(openArgs.value.getMonth()).toBe(9);
      expect(openArgs.value.getDate()).toBe(20);
    });
  });
});
